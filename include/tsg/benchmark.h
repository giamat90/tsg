...
namespace tsg{
    ...
    uint64_t get_now_time(){
        return std::chrono::high_resolution_clock::duration().count();
    }

    uint64_t get_elapsed_time_to_now(time_t start_time){
        return std::chrono::high_resolution_clock::duration().count() - start_time;
    }
}